      ...
      >
        ...
        <q-dialog v-model="showAssigneeDialog" persistent>
          <q-card style="width: 400px; max-width: 90vw">
            ...
            <q-card-section>
              <div class="text-subtitle2 q-mb-md">Item: {{ item.type || 'Item' }}: {{ item.title || 'Untitled' }}</div>
            ...
            <q-card-actions align="right">
              <q-btn flat label="Close" color="primary" v-close-popup @click="closeAssigneeDialog" />
              <q-btn
                label="Save"
                color="positive"
...
const handleDrop = (newStatus) => {
  if (!canChangeSubitemStatus(draggedItem.value) || !draggedItem.value) return
  if (newStatus === 'done') {
...
const saveAssigneeDetails = () => {
  if (!canEditAssignee.value || !selectedAssignee.value || selectedAssigneeIndex.value === null) return
